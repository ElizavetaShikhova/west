...
class Rogue extends Creature {
    ...
    attack(gameContext, continuation) {
        ...
        const target = oppositePlayer.table.find(card => card);
        if (!target) {
            continuation();
            return;
        }
        if (Object.getPrototypeOf(target) === Object.getPrototypeOf(this)) {
            this.dealDamageToCreature(this.maxPower, target, gameContext, continuation);
            return;
        }
        ...
        for (let card of allCards) {
            if (card && Object.getPrototypeOf(card) === targetPrototype) {
                Rogue.stolenAbilities.forEach(abilityName => {
                    if (targetPrototype.hasOwnProperty(abilityName)) {
                        this[abilityName] = targetPrototype[abilityName];
                        delete targetPrototype[abilityName];
                    }
                });
            }
        }
